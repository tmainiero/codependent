...
Both `\semtex@atom` and `\semtex@atomref` are defined via
`\providecommand` as no-ops, so reading the `.aux` file on
pass 2 does not error.

### Staleness detection

The `.sty` writes a hash of all atom registrations to `.aux`:

```tex
\semtex@auxversion{<content-hash>}
```

The hash is computed from the concatenation of all atom
display numbers (not just a count + date), making it robust
against compensating edits.  The `.sbr` file stores a copy;
the `.sty` compares them at `\AtBeginDocument` on pass 2
...
The generic semtex CLI (`semtex backrefs main.aux`) reads
the `.aux` file and produces `main.sbr`:

1. **Parse `.aux`** — extract `\newlabel` entries (label →
   number mapping), `\semtex@atom` entries (atom list),
   `\semtex@atomref` entries (reference edges).  Follow
   `\@input{sub.aux}` directives recursively for multi-file
   documents.
2. **Resolve labels** — for each `\semtex@atomref{A}{label}`,
   look up the label's number via `\newlabel`.  If the number
   matches a known atom, record edge A → B.  If the label
   resolves to a non-atom (equation, section, figure), skip
   silently — only atom-to-atom references produce back-refs.
3. **Deduplicate** — multiple refs from atom A to the same
   target produce one edge.
4. **Filter self-refs** — if atom A references itself, drop.
5. **Invert** — if A references B, then B is "used by" A.
6. **Write `.sbr`**.
...
\semtex@backref{1.2.2}{Definition}{1.2.5, 2.1.2}
\semtex@backref{1.2.3}{Proposition}{1.2.4}
\semtex@section{2}{Functors}
\semtex@backref{2.1.2}{Definition}{2.2.1, 2.2.3}
```

Format: `\semtex@backref{atom-number}{atom-type}{ref-list}`.
Only atoms with back-refs are included.  Section entries
provide grouping for appendix mode.  Section titles are
extracted from `.aux` `\@writefile{toc}{...}` entries.

Atom names for paragraphs: "Paragraph" (or omitted in inline
mode, where only the number matters).

Special characters in atom names (`#`, `%`, `&`, `{`, `}`)
are sanitized by the CLI before writing.
