...
%% semtex with inline backrefs in gray (user's preferred style)
\usepackage[backref-style=inline,backref-color=gray]{semtex}
\semtextrack{theorem,proposition,lemma,corollary,definition,notation,terminology,example,remark,question}

%% Multi-file compatibility check: load subfiles and standalone.
%% These must not conflict with semtex's hooks.
\usepackage{subfiles}
...
\semtexnewcommand{\Funct}[2]{\mathrm{#1}\colon #2}
\semtexnewcommand{\catname}[1]{\mathsf{#1}}
\semtexnewcommand{\Tr}[0]{\operatorname{Tr}}

%% ---------- Equation numbering by section ----------
\numberwithin{equation}{section}

\title{Semtex + sty-theorems-ta stress test}
\author{Test}
\date{}
...
\semtextag{uid}{cat:enriched-category}%
\end{definition}
...
%% --- 4.4: Separated proof with \semtexproofof ---
%%
%% The proof of prop:square is given here, in a different section
%% from the statement.  This tests the \semtexproofof mechanism.
\begin{proof}[\semtexproofof{prop:square}]
...
%% --- 4.5: Multiple cross-references in one atom ---
...
%% --- 4.6: Adjacent proof with cross-references (tests starred backrefs) ---
\begin{proof}
