...
| testfiles/unit/selftest-codependent-exempt.lvt | LIVE | B-WARN-ATTR-CONFLICTS | Verify that Package codependent Warning blocks bypass the undeclared-warning gate |
| testfiles/unit/selftest-multiphase-final-log.lvt | LIVE | B-PASS-ONE | Verify gate uses final-pass log only; pass-1 warnings absent from pass-2 log do not fail |
| testfiles/unit/selftest-repeating-directives.lvt | LIVE | B-PASS-ONE | Verify REPEATING_KEYS registration accumulates multiple REQUIRES directives |
| testfiles/unit/selftest-requires-matches-passes.lvt | LIVE | B-PASS-ONE | Verify that a TEST-REQUIRES-WARNING regex matching a final-log block passes the gate |
| testfiles/unit/selftest-requires-stale-fails.lvt | LIVE | B-PASS-ONE | Verify that a stale TEST-REQUIRES-WARNING (regex never matches) fails the runner gate |
| testfiles/unit/selftest-tolerates-absent-passes.lvt | LIVE | B-PASS-ONE | Verify that TOLERATES absence is OK; gate does not fail when tolerated warning absent |
| testfiles/unit/selftest-undeclared-fails.lvt | LIVE | B-PASS-ONE | Verify that undeclared warning blocks in the final log trigger gate failure |
| testfiles/unit/selftest-wrapped-unwrap.lvt | LIVE | B-PASS-ONE | Verify that max_print_line=10000 prevents log soft-wrap so warning regex can match |
