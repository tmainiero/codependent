...
for FILE in "${FILES[@]}"; do
  ...
  ASSERT_COUNT=$(grep -cP '^%% TEST-(EXIT|LOG-NOT|LOG-CONTAINS|CDP-CONTAINS|CDP-NOT-CONTAINS|CDP-COUNT|CDP-LAST-RECORD|AUX-CONTAINS|AUX-NOT-CONTAINS|ATOMS-MIN|PDF-CONTAINS|PDF-NOT|PDF-LINKS|PDF-STEXT|PDF-STEXT-NOT|PDF-OBJECTS|PDF-OBJECTS-NOT|PDF-LINK-DEST|PDF-LINK-DEST-NOT|PDF-LINK-COUNT|PDF-DEST-EXISTS|PDF-DEST-NOT-EXISTS|PDF-LINK-RECT|PDF-NO-ORPHAN-LINKS|PDF-ALL-BACKREFS-LINKED|PDF-BACKREF-TARGETS|EXPECT-PACKAGE-WARNING|EXPECT-PACKAGE-ERROR|PDF-APPENDIX-ENTRY|PDF-APPENDIX-ENTRY-TEXT-ONLY|PDF-VSPACE-BETWEEN|PDF-LAYOUT-PARITY|REQUIRES-WARNING|TOLERATES-WARNING):' "$FILE" || true)
...
# Validate TEST-REQUIRES/TOLERATES-WARNING annotation quality and ratchet.
if ! python3 .claude/scripts/lint_fixture_warnings.py; then
  ERRORS=$((ERRORS + 1))
fi
